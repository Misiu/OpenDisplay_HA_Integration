...
NRF52840 OTA uses the Adafruit nRF52 bootloader which implements the Nordic
Secure DFU protocol over BLE (service UUID 0xFE59).  The bootloader **requires**
a DFU package (``.zip`` generated by ``adafruit-nrfutil``) that contains:

- An **init packet** (``.dat``) — protobuf-encoded metadata (firmware hash,
  size, device type) that the bootloader validates before accepting data.
- A **firmware binary** (``.bin``) — the application image to flash.

The ``.hex`` (Intel HEX) and ``.uf2`` (USB drag-and-drop) release assets do
**not** contain the init packet and therefore cannot be used for BLE DFU.
The ``.zip`` is both the correct and the smallest download (~189 KB vs ~531 KB
for ``.hex``).

Flow:
    1. HA sends command 0x0044 → device ACKs, resets into DFU bootloader
    2. HA scans for DFU bootloader (service UUID 0xFE59)
    3. HA connects and performs Nordic DFU: init packet → firmware data
    4. Device reboots into updated application firmware
"""
import asyncio
import io
import logging
